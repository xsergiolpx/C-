...
//Variables globales
double h=0.1;
int tam=1; //tamano de los vectores
...
void runge(vector<double> &t, vector<double> &x, vector<double> &y, vector<double> &y4, vector<double> &x4, double m, string nombre);
...
int main(int argc, char **argv)
{
	...
	cout << endl;
        ...
        string nombre="esfera-hierro-macizo";
        double r=0.1;
        double p=7874;
        double m;
        m=masa(p,r);
	runge(t,x,y,y4,x4,m,nombre);
        
        //Esfera hueca de hierro
        nombre="esfera-hierro-hueco";
        double r2=0.09;
        m=masa(p,r)-masa(p,r2);
	runge(t,x,y,y4,x4,m,nombre);
...
void runge(vector<double> &t, vector<double> &x, vector<double> &y, vector<double> &y4, vector<double> &x4, double m, string nombre){
	...
	while(x[i]<0){
		...
		kx1=h*fx(t[i],x[i],y[i]);
                ky1=h*fy(t[i],x[i],y[i],m);
		kx2=h*fx(t[i]+h/2,x[i]+kx1/2,y[i]+ky1/2);
                ky2=h*fy(t[i]+h/2,x[i]+kx1/2,y[i]+ky1/2,m);
		kx3=h*fx(t[i]+3/8*h,x[i]+3*kx1/32+9*kx2/32,y[i]+3*ky1/32+9*ky2/32);
		ky3=h*fy(t[i]+3/8*h,x[i]+3*kx1/32+9*kx2/32,y[i]+3*ky1/32+9*ky2/32,m);
		kx4=h*fx(t[i]+12*h/13,x[i]+1932*kx1/2197-7200*kx2/2197+7296*kx3/2197,y[i]+1932*ky1/2197-7200*ky2/2197+7296*ky3/2197);
                ky4=h*fy(t[i]+12*h/13,x[i]+1932*kx1/2197-7200*kx2/2197+7296*kx3/2197,y[i]+1932*ky1/2197-7200*ky2/2197+7296*ky3/2197,m);
                kx5=h*fx(t[i]+h,x[i]+439*kx1/216-8*kx2+3680*kx3/513-845*kx4/4104,y[i]+439*ky1/216-8*ky2+3680*ky3/513-845*ky4/4104);
		ky5=h*fy(t[i]+h,x[i]+439*kx1/216-8*kx2+3680*kx3/513-845*kx4/4104,y[i]+439*ky1/216-8*ky2+3680*ky3/513-845*ky4/4104,m);
		...
		ky6=h*fy(t[i]+h/2,x[i]-8*kx1/27+2*kx2-3544*kx3/2565+1859*kx4/4104-11*kx5/40,y[i]-8*ky1/27+2*ky2-3544*ky3/2565+1859*ky4/4104-11*ky5/40,m);
